...
namespace Terraria.UI
{
    class UIItemStack :UIBase
    {
        ...
        public UIItemStack(InfoItem infoItem, int count)
        {
            ...
            var rectSize = (Vector2f)Content.texUIInventoryBack.Size;
            rectShape = new RectangleShape(rectSize);
            rectShape.FillColor = Color.Transparent;

            var imgSize = new Vector2f(infoItem.SpriteSheet.SubWight, infoItem.SpriteSheet.SubHeight);
        ...
        {
            if (Parent != null && Parent is UIInventoryCell)
                (Parent as UIInventoryCell).ItemStack = null;
